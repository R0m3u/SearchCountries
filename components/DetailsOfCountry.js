import selector, { selectAll } from "../utils/selector.js";
import Data from "../utils/data.js";
import HomePage from "./Home.js";

export default function DetailsOfCountry(details, names) {
  return {
    onMount() {
                    ...
                    <div class="informations">
                        ...
                        <div class="countries-aside">
                            ...
                            ${names
                              .map(
                                (item) =>
                                  `<button class="border-details">${item.name}</button>`
                              )
                              .join("")}      
    ...
    async onClick() {
      ...
      selector(".back-to-home").addEventListener("click", () => 
        HomePage().onMount()

      );

      selectAll(".border-details").forEach((el, i) =>
        el.addEventListener("click", (e) => {
          let countryInfo = names.filter(
            (border) => border.name === e.target.innerText
          );
          let borders = countryInfo[0].borders;
          let update = data.filter((el) =>
            borders.some((x) => x.includes(el.alpha3Code))
          );

          DetailsOfCountry(countryInfo[0], update).onMount().onClick();
        })
      );
      return this;
    },
